...
public class LeftmostColumnLeastOne {
    ...
    public int solution(BinaryMatrix binaryMatrix) {
        int index = 11111;
        for (int i=0; i < binaryMatrix.dimensions().get(0); i++) {
            index = search(binaryMatrix, i, 0, binaryMatrix.dimensions().get(1) - 1, index);
        }

        return index == 11111 ? -1 : index;
    }

    private int search(BinaryMatrix binaryMatrix, int row, int i, int j, int index) {
        if (i > j) {
            return index;
        }

        int middle = i + (j - i) / 2;
        int value = binaryMatrix.get(row, middle);
        if (value == 1) {
            return Math.min(index, search(binaryMatrix, row, i, middle - 1, middle));
        } else {
            return Math.min(index, search(binaryMatrix, row, middle + 1, j, index));
        }
    }
    ...
    public static class BinaryMatrixImpl implements BinaryMatrix {
